...
class RightWristMechanismTest : public ::testing::Test {
protected:
    virtual void SetUp()
    {
        rightWrist = boost::make_shared<WristMechanism>();
...
//        sliderCalKnown = Vector2f(46, 46);
        ...
        rightWrist->loadDesignParams(A,A0,B,C,D,D0,Pitch,Yaw,M,N,linkLength);
        ...
        rightWrist->setLimits(upperPitch,lowerPitch,upperYaw,lowerYaw);

        SliderGain(0) = .003906;
        SliderGain(1) = .003906;
        rightWrist->setSliderOffsetGain(Vector2f::Zero(), SliderGain);
        rightWrist->setAngleOffsetGain(Vector2f::Zero(), Vector2f(1,1));
        rightWrist->setLimitOffsetGain(1.2,-.586, -1.2, 0, 1.8, 1.34, -.26, 0 );
        rightWrist->eps = .0001;
        rightWrist->maxIt = 1000;
    ...
    boost::shared_ptr<WristMechanism> rightWrist;
...
TEST_F(RightWristMechanismTest, setBadParamsTest)
{
    ...
    rightWrist->outputDesignParams();
    //A and A0 cannot be the same
    EXPECT_THROW(rightWrist->loadDesignParams(A,A,B,C,D,D0,Pitch,Yaw,M,N,linkLength), std::runtime_error);

    //D and D0 cannot be the same
    EXPECT_THROW(rightWrist->loadDesignParams(A,A0,B,C,D,D,Pitch,Yaw,M,N,linkLength), std::runtime_error);

    Vector3f Zero = Vector3f::Zero();;

    //M cannot be a zero vector
    EXPECT_THROW(rightWrist->loadDesignParams(A,A0,B,C,D,D0,Pitch,Yaw,Zero,N,linkLength), std::runtime_error);

    //N cannot be a zero vector
    EXPECT_THROW(rightWrist->loadDesignParams(A,A0,B,C,D,D0,Pitch,Yaw,M,Zero,linkLength), std::runtime_error);
...
TEST_F(RightWristMechanismTest, applyLimitsTest)
{
    ...
    float pitch = 0;
    float yaw = 0;
    rightWrist->applyLimits(pitch, yaw);
...
{
    rightWrist->calWrist(encoder, sliderZeroKnown, Vector2f::Zero());

    std::cout<<"angleCalKnown: (0,0)"<<std::endl;
    Vector2f sliderCalPos = rightWrist->getSliderFromAngle(Vector2f::Zero());
    std::cout<<"slider pos:"<<std::endl<<sliderCalPos<<std::endl;

    Vector2f angOut = rightWrist->getAngleFromSlider(sliderCalKnown);
    std::cout<<"angle at cal pos: "<<std::endl<<angOut<<std::endl;
}
...
TEST_F(RightWristMechanismTest, getSliderFromAngleTest)
{
    ...
    rightWrist->calWrist(encoder,  sliderCalKnown, angleCalKnown);

    //check cal position
    std::cout<<"angleCalKnown: ("<<angleCalKnown(0)<<","<<angleCalKnown(1)<<")"<<std::endl;
    Vector2f sliderCalPos = rightWrist->getSliderFromAngle(angleCalKnown);
    std::cout<<"slider pos:"<<std::endl<<sliderCalPos<<std::endl;
    EXPECT_NEAR(sliderCalKnown(0), sliderCalPos(0),  1);
    EXPECT_NEAR(sliderCalKnown(1), sliderCalPos(1),  1);

    //check zero position
    Vector2f sliderZeroPos = rightWrist->getSliderFromAngle(Vector2f::Zero());
    ...
    Vector2f slider = rightWrist->getSliderFromAngle(ang);
    ...
    ang(0) = -0.0872664626; //-5 degrees
    slider = rightWrist->getSliderFromAngle(ang);
    std::cout<<"slider pos:"<<std::endl<<slider<<std::endl;
    EXPECT_GT(sliderZeroKnown(0), slider(0));
    EXPECT_GT(sliderZeroKnown(1), slider(1));

    ang = Vector2f::Zero();
    //change yaw -- increase of yaw should decrease thumbside and increase littleside
    ...
    ang = Vector2f::Zero();
    //change yaw -- decrease of yaw should increase thumside and decrease littleside
...
TEST_F(RightWristMechanismTest, sliderWristConversionTest)
{
    ...
    rightWrist->calWrist(encoder, slider, ang);

    //test forward and back give same result
    Vector2f slider2 = rightWrist->getSliderFromWristEncoder(encoder);
    EXPECT_FLOAT_EQ(slider(0), slider2(0));
    EXPECT_FLOAT_EQ(slider(1), slider2(1));

    Vector2f encoder2 = rightWrist->getWristEncoderFromSlider(slider);
    ...
    Vector2f sliderInc = rightWrist->getSliderFromWristEncoder(encoderInc);
    ...
    Vector2f sliderDec = rightWrist->getSliderFromWristEncoder(encoderDec);
    ...
    encoderInc = rightWrist->getWristEncoderFromSlider(sliderInc);
    ...
    encoderDec = rightWrist->getWristEncoderFromSlider(sliderDec);
...
TEST_F(RightWristMechanismTest, NewtonsMethodTest)
{
    ...
    rightWrist->eps = 0.0001;
    Vector2f slider = rightWrist->getSliderFromAngle(angKnown);
    Vector2f correction;
    for(int i = 0; i<10; ++i)
    {
        correction = rightWrist->NewtonsMethod(ang, slider);
        ang = ang - correction;
    }
...
TEST_F(RightWristMechanismTest, getAngleFromSliderTest)
{
    ...
    EXPECT_NO_THROW(rightWrist->calWrist(encoder,  sliderCalKnown, angleCalKnown));
    rightWrist->maxIt = 150;
    rightWrist->eps = .0001;

    Vector2f angOut;
    //check calibration point
    EXPECT_NO_THROW(angOut = rightWrist->getAngleFromSlider(sliderCalKnown));
    ...
    //check zero pos
    EXPECT_NO_THROW(angOut = rightWrist->getAngleFromSlider(sliderZeroKnown));
    std::cout<<"angle:"<<std::endl<<angOut<<std::endl;
    ...
    EXPECT_NO_THROW(angOut = rightWrist->getAngleFromSlider(slider));
    std::cout<<"angle:"<<std::endl<<angOut<<std::endl;
    ...
    //decreasing thumbside, and increasing littleside should increase yaw
    slider = sliderZeroKnown + Vector2f(-5,5);
    EXPECT_NO_THROW(angOut = rightWrist->getAngleFromSlider(slider));
    std::cout<<"angle:"<<std::endl<<angOut<<std::endl;
    EXPECT_LT(angZeroKnown(1), angOut(1));

    //increasing thumbside, and decreasing littleside should decrease yaw
    slider = sliderZeroKnown + Vector2f(5,-5);
    EXPECT_NO_THROW(angOut = rightWrist->getAngleFromSlider(slider));
    std::cout<<"angle:"<<std::endl<<angOut<<std::endl;
    ...
    for(double i = lowerPitch; i<upperPitch; i+=.01)
    {
        for(double j = lowerYaw; j< upperYaw; j+=.01)
        {
            try
            {
                ...
                float yaw = j;
                rightWrist->applyLimits(pitch,yaw);
                sliderOut = rightWrist->getSliderFromAngle(Vector2f(pitch,yaw));
            ...
            try
            {
                angOut = rightWrist->getAngleFromSlider(sliderOut);
                float pitch = i;
                float yaw = j;
                rightWrist->applyLimits(pitch, yaw);
...
////                    std::cin.ignore();
//                }
                EXPECT_NEAR(pitch, angOut(0), 0.0005);
                EXPECT_NEAR(yaw, angOut(1), 0.0005);
...
/*
TEST_F(RightWristMechanismTest, FindRangeSpaceTest)
{

//    for(double i = 0; i < -100; i = i-.1)
//    {
//        std::cout<<"("<<i<<","<<i<<")"<<std::endl;
//        EXPECT_NO_THROW(rightWrist->getAngleFromSlider(Vector2f(i,i)));
//        rightWrist->getAngleFromSlider(Vector2f(i,i));
//    }
    ...
    for(double i = -25; i < 95; i = i+.1)
    {
        ...
        EXPECT_NO_THROW(angOut = rightWrist->getAngleFromSlider(Vector2f(i,i)));
        try
        {
            angOut = rightWrist->getAngleFromSlider(Vector2f(i+25,i));
    ...
    for(double i = -25; i < 95; i = i+.1)
    {
        ...
        {9
            ...
            //EXPECT_NO_THROW(rightWrist->getAngleFromSlider(Vector2f(i,j)));
            try
            {
                angOut = rightWrist->getAngleFromSlider(Vector2f(i,j));

                if(maxPitch(0)<angOut(0))
                    maxPitch = angOut;
                if(minPitch(0)>angOut(0))9
                    minPitch = angOut;
                if(maxYaw(1)<angOut(1))
                    maxYaw = angOut;
                if(minYaw(1)>angOut(1))
    ...
    for(double i = -25; i < 95; i = i+.1)
    {
        for(double j = i-25; j<i+25; j = j+.1)
        {
            ...
            //EXPECT_NO_THROW(rightWrist->getAngleFromSlider(Vector2f(j,i)));
            try
            {
                angOut = rightWrist->getAngleFromSlider(Vector2f(j,i));
                if(maxPitch(0)<angOut(0))
                    maxPitch = angOut;
                if(minPitch(0)>angOut(0))
                    minPitch = angOut;
...
//TEST_F(RightWristMechanismTest, RangeSpaceTest)
//{
...
//        for(double j = i-25; j<i+25; j = j+.1)
//        {
//            EXPECT_NO_THROW(angOut = rightWrist->getAngleFromSlider(Vector2f(i,j)));
//            if(maxPitch(0)<angOut(0))
//                maxPitch = angOut;
//            if(minPitch(0)>angOut(0))
//                minPitch = angOut;
...
//        for(double j = i-25; j<i+25; j = j+.1)
//        {
//            EXPECT_NO_THROW(angOut = rightWrist->getAngleFromSlider(Vector2f(j,i)));
//            if(maxPitch(0)<angOut(0))
//                maxPitch = angOut;
//            if(minPitch(0)>angOut(0))
//                minPitch = angOut;
...
TEST_F(RightWristMechanismTest, calibration)
{
    ...
    Vector2f sliderTest = rightWrist->getSliderFromAngle(angleCalKnown);
    ...
    Vector2f sliderOut = rightWrist->getSliderFromWristEncoder(encoder);
